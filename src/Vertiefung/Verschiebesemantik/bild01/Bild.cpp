...
Bild::Bild(unsigned int b, unsigned int h, const string& name) : b(b), h(h), name(name) {
	cout << "Bild(): \"" << name << "\"\n";
	p = new int[b*h];
	for(unsigned int i=0; i<b*h; i++) { p[i]=0; }
}
Bild::~Bild() {
	cout << "~Bild(): \"" << name << "\"\n";
...
void Bild::setzePixel(unsigned int x, unsigned int y, int farbwert) {
	p[x+y*b] = farbwert;
}
ostream& operator<<(ostream& os, const Bild& r) {
	...
	for(unsigned int y=0;y<r.h;y++) {
		os << "[ ";
		for(unsigned int x=0;x<r.b;x++) { os << r.p[i++] << " "; }
		os << "]";
	}
	return os;
}
